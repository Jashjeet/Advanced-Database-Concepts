...
--help taken from Harsha in this query	
    insert into result 
	with tc1 as 
...
drop table if exists powerset_t;
create table powerset_t as (with recursive powerset as 
						  (select '{}'::text[] as subset  
						   union      
						   select array(select * from UNNEST(subset) 
										union 
										select word order by 1)      
						   from   powerset S, unique_words u     
...
for i in (select * from powerset_t)
loop

insert into result
select i.subset,count(1)
from documents d
where i.subset<@ d.words;
...
drop table documents,powerset_t,result;
